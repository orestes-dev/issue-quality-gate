...
in CI (`skipConventionalCommits`, `maxAllowedEmDashes`, `allowEmDashes`,
`allowDefaultBranchCommits`), and the local commit hooks, which ship separately.
Both key off the same committed file, so a repo's baseline relaxations are the
...
and the thin workflows `templates/workflow/issue-quality.yml`,
`pr-readiness.yml`, and `commit-hygiene.yml`. This repo's own `.github/` and root
`.template.{issue,pr}.md` are a dogfood instance of that bundle: the applied Forms
and Author guides are drift-tested byte-identical to the canonical ones, and each
dogfood workflow is drift-tested to agree with its consumer template on every
shared field (they differ only on `uses: ./` vs `@main`).
